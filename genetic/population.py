...
class Gene:
    ...
    def generate_gene(self, start_point, end_point):
        current_p = Point(start_point.x, start_point.y)
        last_direction = -1
        while not current_p.__eq__(end_point):
            new_data = self.generate_next_segment(current_p, end_point, last_direction)
            last_direction = new_data[1]
            current_p.update_point_coordinates(new_data[0])

    def generate_next_segment(self, current_point, end_point, last_dir):
        ...
        if current_point.x == end_point.x:
            if last_dir != Direction.VERTICAL:
                ...
                # TODO zastanowić sie nad dlugoscia tego odcinka (jak ja dobrac)
                current_dir = Direction.HORIZONTAL
                segment_len = random.randint(1, 3)
    ...
    def calculate_outside_length(point_x, point_y, step, direction, plate_x, plate_y):
        ...
        for i in range(step):
            ...
            if direction == Direction.RIGHT:
                point_x += 1
            if point_x > plate_x or point_y > plate_y or point_y <= 0 or point_x <= 0:
                length += 1
        return length
...
class Individual:
    ...
    def generate_genotype(self, connections):
        for connection in connections:
            gene = Gene()
            gene.generate_gene(connection.start_point, connection.end_point)
            self.genotype.append(gene)

    def get_intersection_points(self, connections):
        crossing_connections_by_point = dict()
        crossing_connections = list()
        ...
        for cross in crossing_connections_by_point.values():
            if len(cross) > 1:
                crossing_connections.append(cross)
    ...
    def get_total_outside_paths_number(self, start_point, plate_x, plate_y):
        total = 0
        for gene in self.genotype():
            if gene.is_path_outside(start_point, plate_x, plate_y):
                total += 1
        return total
    ...
    def get_total_outside_path_length(self, start_point, plate_x, plate_y):
        total = 0
        for gene in self.genotype():
            total += gene.get_outside_path_length(start_point, plate_x, plate_y)
        return total

    def get_intersections_penalty(self, connections):
...
class Solution:
        ...
        with open(filename) as fp:
            for count, line in enumerate(fp):
                ...
                if count == 0:
                    ...
                    self.connections.append(Connection(Point(int(data[0]), int(data[1])),
    ...
    def __str__(self):
        ...
        return f"Plate: {self.plate_x}x{self.plate_y}, Connections:\n{conns}"


if __name__ == '__main__':
    solution = Solution()
    solution.read_solution_from_file('zad0.txt')
    # print(solution)
    ind = Individual()
    ind.generate_genotype(solution.get_connections())
    print(ind.get_intersections_penalty(solution.get_connections()))
    # ind.print_genes()
